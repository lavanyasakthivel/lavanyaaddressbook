...
void SQLiteDataSource::fillContactFromRow(sqlite3_stmt *s, Contact& c)
{
    ...
    c.email = reinterpret_cast<const char*>(sqlite3_column_text(s, 5));
     ...
     c.country = reinterpret_cast<const char*>(sqlite3_column_text(s, 7));
}
...
{
    //create sql prepared statement
    std::string sqlStr = "SELECT * FROM Contacts2;";
...
{
    //create sql prepared statement
    std::string sqlStr = "INSERT INTO Contacts2 VALUES("
                        "NULL,?,?,?,?,?,?,?);";
    ...
    sqlite3_bind_text(insertStatement.get(), 7, c.country.c_str(), -1, SQLITE_STATIC);
...
{
    //create sql prepared statement
    std::string sqlStr = "UPDATE Contacts2 SET "
                         "firstname=?, lastname=?,"
                         "phonenum=?, address=?,"
                         "email=?,city=?,"
                         "country=? WHERE id=?";
    ...
    sqlite3_bind_text(updateStatement.get(), 7, c.country.c_str(), -1, SQLITE_STATIC);
